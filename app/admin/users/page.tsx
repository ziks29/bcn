...
export default async function UsersPage() {
    ...
    return (
        <div className="p-4 md:p-8 font-serif-body">
            <Link href="/admin" className="text-zinc-500 hover:text-black mb-4 inline-block text-sm font-bold uppercase tracking-widest">← Назад в панель</Link>
            <div>
                <h1 className="font-headline text-2xl sm:text-3xl md:text-4xl uppercase tracking-tighter mb-6 md:mb-8 bg-black text-white p-3 md:p-4 inline-block transform -rotate-1">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
                {/* Create User Form */}
                <div className="bg-white border-2 border-black p-4 md:p-6 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] md:shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] h-fit">
                    <h2 className="font-headline text-xl sm:text-2xl uppercase mb-4 border-b-2 border-black pb-2">Добавить сотрудника</h2>
                    <CreateUserForm />
                </div>
                                    ...
                                    <span className={`text-xs font-bold px-2 py-0.5 text-white ${user.role === 'ADMIN' ? 'bg-red-600' :
                                        user.role === 'CHIEF_EDITOR' ? 'bg-purple-600' :
                                            user.role === 'EDITOR' ? 'bg-blue-600' :
                                                user.role === 'DISABLED' ? 'bg-zinc-600' : 'bg-green-600'
                                        }`}>
                                        {user.role}
                                    </span>
                                    {!user.approved && (
